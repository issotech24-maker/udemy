...
function VerifiedBadge() {
  return (
    <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-sm">
      {/* Pulsing dot — signals live automated verification */}
...
function CouponCard({ coupon }: { coupon: Coupon }) {
  ...
  return (
    <article className="bg-white border border-slate-200 rounded-md flex flex-col hover:border-slate-300 transition-colors overflow-hidden h-full">
      {/* Top bar */}
  ...
  return (
    <div className="space-y-7">
      ...
      {visible.length === 0 ? (
        ...
        /*
         * key={active} remounts the grid on every filter change so that
         * each new set of cards gets a fresh CSS animation cycle.
         */
        <div
          key={active}
